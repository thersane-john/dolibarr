...
function show_actions_done($conf, $langs, $db, $filterobj, $objcon = null, $noprint = 0, $actioncode = '', $donetodo = 'done', $filters = array(), $sortfield = 'a.datep,a.id', $sortorder = 'DESC', $module = '')
{
	...
	if (isModEnabled('agenda') || (isModEnabled('mailing') && !empty($objcon->email))) {
		...
		$out .= $form->select_dolusers((($filters['search_filtert']??0) > 0 ? $filters['search_filtert'] : ''), 'search_filtert', 1, null, (int) !$canedit, '', '', '0', 0, 0, '', 2, '', 'minwidth100 maxwidth250 widthcentpercentminusx');
		$out .= '</td>';
		// Type
		$out .= '<td class="liste_titre">';
