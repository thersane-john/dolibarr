...
if ($action == 'create') {
	...
	if (empty($reshook)) {
		...
		$form->selectInputReason((GETPOSTINT('demand_reason_id') ? GETPOSTINT('demand_reason_id') : ''), 'demand_reason_id', "SRC_PROP", 1, 'maxwidth200 widthcentpercentminusx');
		print '</td></tr>';
	...
	if ($action != 'presend') {
		...
		if ($user->hasRight('propal', 'creer') && $object->statut == Propal::STATUS_DRAFT) {
			$compatibleImportElementsList = array('commande', 'propal', 'facture', 'propal'); // import from linked elements
		}
